...
function routeForLocation(): Route {
  if (new URLSearchParams(location.search).get('demo') === '1' || location.pathname === '/demo') return 'demo';
...
$('#expand-all').addEventListener('click', (event) => { const details = result.querySelectorAll('details'); const shouldOpen = [...details].some((item) => !item.open); details.forEach((item) => item.open = shouldOpen); (event.currentTarget as HTMLButtonElement).textContent = shouldOpen ? 'Collapse all' : 'Expand all'; });
function runPlanner() {
...
function renderPlan(plan: BrowserPlan) {
  ...
  result.innerHTML = `<div class="summary"><span>Workflow <strong>${escape(plan.decision.outcome.toUpperCase())}</strong></span><span><strong>${jobs}</strong> ${jobs === 1 ? 'job' : 'jobs'}</span><span><strong>${cells}</strong> ${cells === 1 ? 'cell' : 'cells'}</span><span><strong>${steps}</strong> ${steps === 1 ? 'step' : 'steps'}</span></div><ul class="plan-tree"><li class="plan-node"><div class="node-row"><span class="status-dot ${plan.decision.outcome}" aria-hidden="true"></span><span class="node-title">${escape(plan.name)} <small>workflow</small></span><span class="decision">${escape(plan.decision.reason)}</span></div>${renderJobs(plan)}</li></ul>${renderMeta(plan)}`;
}
function renderJobs(plan: BrowserPlan) { if (!plan.jobs.length) return `<div class="plan-meta">No jobs are planned because the workflow trigger is <strong>${escape(plan.decision.outcome)}</strong>.</div>`; return `<ul class="children">${plan.jobs.map((job) => `<li class="plan-node"><details open><summary class="node-row"><span class="status-dot ${job.decision.outcome}" aria-hidden="true"></span><span class="node-title">${escape(job.name)} <small>${escape(job.id)}${job.needs.length ? ` · needs ${job.needs.map(escape).join(', ')}` : ''}</small></span><span class="decision">${escape(job.decision.reason)}</span></summary><ul class="children">${job.cells.map((cell, index) => `<li><div class="step-row"><span class="status-dot run" aria-hidden="true"></span><span><strong>${Object.keys(cell.values).length ? `Matrix ${index + 1}` : 'Steps'}</strong>${Object.keys(cell.values).length ? ` · <code>${Object.entries(cell.values).map(([k, v]) => `${escape(k)}=${escape(String(v))}`).join(' ')}</code>` : ''}</span></div><ul class="children">${cell.steps.map((step) => `<li class="step-row"><span class="status-dot ${step.decision.outcome}" aria-hidden="true"></span><span><strong>${escape(step.name)}</strong><br />${escape(step.decision.reason)}${step.run?.includes('<unknown:') ? ' · resolved command contains an unknown' : ''}</span></li>`).join('')}</ul></li>`).join('')}</ul></details></li>`).join('')}</ul>`; }
...
function renderEmpty(message = 'Your explained workflow plan will appear here.') { result.innerHTML = `<div class="empty-state"><div class="empty-mark" aria-hidden="true">⌁</div><h4>Ready when the workflow is</h4><p>${escape(message)} Press “Show this workflow’s plan” or <kbd>⌘ ↵</kbd>.</p></div>`; }
