# No-execution GitHub Actions workflow planner: which jobs/steps will run for a given event, and why (`gha-dry-run-planner`) — factory product contract

This repository is one product built by the Param Factory. It ships at
`https://gha-dry-run-planner.sociobot.in`. Artifact class: `cli`.
